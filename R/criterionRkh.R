...
n<-length(Y)
p<-ncol(X)
if(missing(H)) H<-round(seq(2,n/4,l=8))
if(missing(K)) K <- 1:(min(p,25))
...
if (missing(indices)){indices<-sample(n,B*n,replace=T)}
else{if(length(indices)!=(B*n)){
indices<-sample(n,B*n,replace=T)
cat("The bootstrap sample is generating", "\n")}}

projector <-function(beta1,sigm){beta1%*%(solve(t(beta1)%*%sigm%*%beta1))%*%t(beta1)%*%sigm}
...
res <- list(Rkhbootmean=Re(resRkhboot),Rkhboot=simboot,method=method,n=n,H=H,K=K,indices=indices)
